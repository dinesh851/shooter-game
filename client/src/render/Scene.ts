...
export class Scene {
  ...
  private buildViewModel(): THREE.Group {
    ...
    // weapon. Procedural low-poly to match the stylized look; they live in the
    // viewModel group so they sway/recoil/ADS with the gun and are hidden by the
    // same alive/scoped visibility logic.
    const right = this.makeArm(); // trigger hand, near the grip
    right.position.set(0.04, -0.07, 0.05);
    right.rotation.set(0.55, 0.12, -0.18);
    const left = this.makeArm(); // support hand, forward on the foregrip
    left.position.set(-0.05, -0.06, -0.26);
    left.rotation.set(0.75, -0.12, 0.28);
    g.add(right, left);
  ...
  // one procedural first-person arm: a fist at the group origin (sits on the gun)
  // with a sleeved forearm trailing down toward the bottom of the screen
  private makeArm(): THREE.Group {
    const skin = new THREE.MeshStandardMaterial({ color: 0xc99a6b, roughness: 0.85, metalness: 0, flatShading: true });
    const sleeve = new THREE.MeshStandardMaterial({ color: 0x39513e, roughness: 0.95, metalness: 0, flatShading: true });
    const arm = new THREE.Group();
    const fist = new THREE.Mesh(new THREE.IcosahedronGeometry(0.052, 0), skin);
    fist.scale.set(1, 0.85, 1.15);
    const wrist = new THREE.Mesh(new THREE.CylinderGeometry(0.038, 0.044, 0.07, 8), skin);
    wrist.position.set(0, -0.06, 0.01);
    const fore = new THREE.Mesh(new THREE.CylinderGeometry(0.044, 0.058, 0.34, 8), sleeve);
    fore.position.set(0, -0.24, 0.02);
    arm.add(fist, wrist, fore);
    arm.traverse((o: any) => {
      if (o.isMesh) {
